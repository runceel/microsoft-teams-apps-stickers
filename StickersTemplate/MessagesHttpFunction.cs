...
public class MessagesHttpFunction
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        using (var scope = logger.BeginScope($"{nameof(MessagesHttpFunction.Run)}"))
        {
            ...
            var stickerSet = await _stickerSetRepository.FetchStickerSetAsync();
            await _stickerSetIndexer.IndexStickerSetAsync(stickerSet);
            var stickers = await _stickerSetIndexer.FindStickersByQuery(query, skip, count);
